        ...
        loader.load( url, ( buffer ) => {
            ...
            this._createTexture( [ buffer ] )
                .then( function ( _texture ) {
                    ...
                    if ( _texture.constructor !== texture.constructor ) {
                        texture = new _texture.constructor();
                    }
    ...
    _createTexture( buffers, config = {} ) {
        ...
        const texturePending = this._allocateWorker( taskCost )
            ...
            .then( ( message ) => {
                ...
                const { mipmaps, cubeImages, width, height, format } = message;

                if ( cubeImages ) {

                    // Todo : Handle mimmaps for CubeTexture
                    const textures = [];

                    for ( let i = 0; i < cubeImages.length; i ++ ) {

                        const texture = new CompressedTexture( [ cubeImages[ i ] ], width, height, format, UnsignedByteType );
            ...
            case 'transcode':
                transcoderPending.then( () => {
                    ...
                    try {
                        ...
                        const { width, height, hasAlpha, mipmaps, cubeImages, format } = message.taskConfig.lowLevel
                        ...
                        if ( cubeImages ) {
                            ...
                            for ( let i = 0; i < cubeImages.length; i ++ ) {
                                buffers.push( cubeImages[ i ].data.buffer );
                            }

                            self.postMessage( { type: 'transcode', id: message.id, width, height, hasAlpha, cubeImages, format }, buffers );
    ...
    function transcode( buffer ) {
        ...
        // Cubemaps
            ...
            for ( let i = 0; i < numImages; i ++ ) {
                ...
                const mip = 0;
                const cubeImageWidth = basisFile.getImageWidth( i, mip );
                const cubeImageHeight = basisFile.getImageHeight( i, mip );
                const dst = new Uint8Array( basisFile.getImageTranscodedSizeInBytes( i, mip, transcoderFormat ) );
                const status = basisFile.transcodeImage(
                    dst,
                    i,
                    mip,
                ...
                cubeImages.push( { data: dst, width: cubeImageWidth, height: cubeImageHeight } );

            }

            cleanup();

            return { width, height, hasAlpha, cubeImages, format: engineFormat };
