...
class ResourceManager extends EventDispatcher {
    ...
    load() {
        ...
        for (let i = 0; i < this._resources.length; i++) {
            ...
            promise.then(this._preloadProgressHandler, this._preloadErrorHandler);
    ...
    _setupResources() {
        ...
        for (let i = 0; i < ResourceLoader.resources.length; i++) {
            const resource = ResourceLoader.resources[i];
            if (resource.namespace !== undefined && resource.namespace === this._namespace) {
                resources.push(resource);
            } else if (this._namespace === undefined && resource.preload) {
                resources.push(resource);
            }
        }
